...
def get_uptime_minutes(script_path):
    ...
    try:
        pids = get_pids_for_script(script_path)
        if not pids:
            return None
...
def can_restart_now():
    ...
    try:
        with open(BACKUP_BOTS_FILE, 'r') as f:
            for line in f:
                line = line.strip()
...
def is_node_script(script_path):
    """True if script is a Node.js file (run with node instead of python)."""
    if not script_path:
        return False
    p = script_path.strip()
    return p.endswith('.js') or p.lower().endswith('.mjs')


def get_node_pids_for(script_path):
    """Get PIDs of node processes running this script."""
    matches = []
    try:
        result = subprocess.run(['pgrep', '-f', f'node.*{script_path}'],
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                if NODE_PROCESS_NAME in (proc.info.get('name') or '') and script_path in ' '.join(proc.info.get('cmdline') or []):
...
def get_pids_for_script(script_path):
    """Get PIDs for a bot (Python or Node) based on file type."""
    if is_node_script(script_path):
        return get_node_pids_for(script_path)
    return get_python_pids_for(script_path)

def start_tmux_session(session_name, script_path):
    """Start tmux session with timeout to prevent hanging. Runs Python or Node based on file type."""
    try:
        if is_node_script(script_path):
            # Run from script directory so Node finds node_modules in cwd or parent
            script_dir = os.path.dirname(os.path.abspath(script_path))
            script_name = os.path.basename(script_path)
            cmd = f'cd "{script_dir}" && node "{script_name}"'
        else:
            cmd = f'python3.11 "{script_path}"'
...
def should_restart_bot_timestamp(script_path, alarm_minutes, restart_minutes, uptime_minutes=None):
    ...
    # Get process start time (when the actual process started)
    process_start_time = get_process_start_time(script_path)
...
def fix_bot(script_path):
    ...
    if not pids:
        ...
        try:
            ...
            time.sleep(0.5)
            new_pids = get_pids_for_script(script_path)
...
def kill_all_bots(bot_list):
    for script in bot_list:
        try:
            for pid in get_pids_for_script(script):
                kill_pid(pid)
