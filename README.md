...
**To get "Information" (signals) and "Binance Data" (live position) on localhost:**

1. **Node API** — in one terminal, from repo root:
   ```bash
   cd server && node server.example.js
   ```
   (Or `node server-local.js` if you use that.) Server must listen on **port 10000**; Vite will proxy `/api` and `/auth` to it.

2. **Python signals API** — in another terminal (required for signals table and open-position data):
   ```bash
   cd python && python3 api_signals.py
   ```
   It should listen on **port 5001**. If your Node server uses a different URL, set `PYTHON_SIGNALS_URL` (e.g. `PYTHON_SIGNALS_URL=http://localhost:5001`).

3. **Refresh the Live Trade page** — "Loading signals..." and "Trade not in exchange — no live data" will clear once the Python API is running and returns data.
