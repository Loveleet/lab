...
// Placeholder so the first chart is always BTC
const BTC_PLACEHOLDER = { Pair: "BTCUSDT", PL: null, Buy_Price: null, Sell_Price: null, Stop_Price: null, Save_Price: null };
...
const ChartGridPage = ({ symbols: initialSymbols = [], trades: initialTrades = [] }) => {
  ...
  // BTC pinned chart + other charts
  const btcTrade = React.useMemo(() => {
    const found = trades.find(t => getRobustSymbol(t.Pair) === "BTCUSDT");
    return found || BTC_PLACEHOLDER;
  }, [trades]);

  const otherTrades = React.useMemo(() => {
    const paged = pagedTrades();
    return paged.filter(t => getRobustSymbol(t.Pair) !== "BTCUSDT");
  ...
  useEffect(() => {
    ...
    const allCharts = [btcTrade, ...otherTrades];

    allCharts.forEach((trade, idx) => {
      const symbol = getRobustSymbol(trade.Pair);
      const containerId = idx === 0 ? "tv_chart_btc" : `tv_chart_${idx - 1}`;
    ...
    // eslint-disable-next-line
  }, [source, interval, indicators, layout, page, chartSize, trades, pageSize, btcTrade, otherTrades]);
  ...
  return (
    <div className="w-full h-full min-h-screen bg-black">
      ...
      <div className="px-6 pt-4">
        <div
          className="bg-[#181818] rounded-lg p-2 flex flex-col items-center shadow relative sticky z-30"
          style={{
            top: filterBarRef.current ? filterBarRef.current.offsetHeight : 56,
        ...
        >
          {(() => {
            const symbol = getRobustSymbol(btcTrade.Pair);
            return (
              <>
                <div className="font-bold mb-1 text-white">
                  {symbol} {source === "tradingview" ? "4c8" : "7e1"} CHART (pinned)
                </div>
                {source === "tradingview" ? (
                  <div id="tv_chart_btc" style={{ width: "100%", height: chartSize.height }} />
                ) : (
                  <iframe
                    title={symbol}
                ...
                <div className="mt-2 text-white w-full flex flex-col gap-1" style={{ fontSize: fontSize }}>
                  <div>PL: <b>{btcTrade.PL ?? 'N/A'}</b></div>
                  <div>
                    <span
                      className="cursor-pointer hover:underline px-1"
                      onClick={() => handleCopy('Buy_Price', btcTrade.Buy_Price, 'btc')}
                      title="Click to copy"
                    >Buy: <b>{btcTrade.Buy_Price ?? 'N/A'}</b>{copied[`btc_Buy_Price`] && <span className="ml-1 text-green-400">Copied!</span>}</span>
                    <span className="mx-1">|</span>
                    <span
                      className="cursor-pointer hover:underline px-1"
                      onClick={() => handleCopy('Sell_Price', btcTrade.Sell_Price, 'btc')}
                      title="Click to copy"
                    >Sell: <b>{btcTrade.Sell_Price ?? 'N/A'}</b>{copied[`btc_Sell_Price`] && <span className="ml-1 text-green-400">Copied!</span>}</span>
                  </div>
                  <div>
                    <span
                      className="cursor-pointer hover:underline px-1"
                      onClick={() => handleCopy('Stop_Price', btcTrade.Stop_Price, 'btc')}
                      title="Click to copy"
                    >Stop: <b>{btcTrade.Stop_Price ?? 'N/A'}</b>{copied[`btc_Stop_Price`] && <span className="ml-1 text-green-400">Copied!</span>}</span>
                    <span className="mx-1">|</span>
                    <span
                      className="cursor-pointer hover:underline px-1"
                      onClick={() => handleCopy('Save_Price', btcTrade.Save_Price, 'btc')}
                      title="Click to copy"
                    >Save: <b>{btcTrade.Save_Price ?? 'N/A'}</b>{copied[`btc_Save_Price`] && <span className="ml-1 text-green-400">Copied!</span>}</span>
                  </div>
                </div>
              </>
            );
