...
const formatTradeData = (trade, index) => ({
    ...
    ...(function () {
      const swingKeys = Object.keys(trade).filter((k) => /^swing\d+$/i.test(k));
      swingKeys.sort((a, b) => {
      ...
      swingKeys.forEach((k) => {
        ...
        out[`Swing${n}`] = safeFixed(trade[k], 6);
      });
      return out;
    })(),
  });
...
const TableView =  ({ title, tradeData, clientData, activeSubReport, setActiveSubReport }) => {
  ...
  const columnOrderInitializedRef = useRef(false);
...
// Sync column order from data + localStorage: first init from saved/default; then always append newly added labels at end
useEffect(() => {
  if (!filteredAndSortedData.length) return;

  const keys = Object.keys(filteredAndSortedData[0]);

  if (!columnOrderInitializedRef.current) {
    let stored = [];
    try {
    ...
    columnOrderInitializedRef.current = true;
    return;
  }

  // Already initialized: append any new keys from data at the end (show all, new labels at end)
  setColumnOrder((prev) => {
    const kept = prev.filter((k) => keys.includes(k) || k === "Live");
    const newKeys = keys.filter((k) => !prev.includes(k));
    if (newKeys.length === 0) return prev;
    const next = [...kept, ...newKeys];
    if (!next.includes("Live")) next.push("Live");
    localStorage.setItem(columnOrderKey, JSON.stringify(next));
    return next;
  });
}, [filteredAndSortedData, columnOrderKey]);
...
useEffect(() => {
  ...
  const getRawTrade = useCallback((formattedRow) => {
    if (!formattedRow || !filteredRawTrades.length) return null;
    const uid = stripForCompare(formattedRow.Unique_ID);
    if (!uid || uid === "n/a") return null;
    return (
      filteredRawTrades.find((t) => {
        const rawUid = stripForCompare(t?.unique_id ?? t?.Unique_ID);
        return rawUid === uid;
      }) || null
    );
  }, [filteredRawTrades]);
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          onClick={() => {
            setActiveFilters({});
            setFilteredData(tradeData);
            if (typeof setSelectedIntervals === "function") {
