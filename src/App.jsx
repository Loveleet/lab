...
const App = () => {
  ...
  const [manageAutoPositionSides, setManageAutoPositionSides] = useState(null); // { buyActive, sellActive } | null
  /** Batch set stop from Binance: preview → confirm → password → one request per pair */
...
const [selectedIntervals, setSelectedIntervals] = useState(() => {
  ...
  const refreshAllData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch Manage Auto Position (enabled + side: BUY | SELL | BOTH | NONE)
      try {
        const mapRes = await apiFetch("/api/manage-auto-position");
        const mapJson = mapRes.ok ? await mapRes.json() : null;
        const sideToBuySell = (side) => {
          const s = (side == null ? "BOTH" : String(side)).trim().toUpperCase();
          if (s === "BUY") return { buyActive: true, sellActive: false };
          if (s === "SELL") return { buyActive: false, sellActive: true };
          if (s === "NONE") return { buyActive: false, sellActive: false };
          return { buyActive: true, sellActive: true };
        };
        if (mapJson && typeof mapJson.enabled === "boolean") {
          setManageAutoPosition(mapJson.enabled);
          setManageAutoPositionSides(sideToBuySell(mapJson.side));
        } else {
...
useEffect(() => {
  ...
  .filter(trade => {
    ...
    return plValue > 0 && isHedge && isHedge11 && (trade.type === "running" || trade.type === "hedge_hold");
  })
  ...
  return (
      ...
      {setAllStopFlow && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="set-all-stop-title"
        >
          <div className="bg-white dark:bg-[#1a1a1a] rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 max-w-lg w-full max-h-[85vh] flex flex-col">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center gap-2">
              <h2 id="set-all-stop-title" className="text-lg font-semibold text-gray-900 dark:text-white">
                Set stop — {setAllStopFlow.title}
              </h2>
              <button
                type="button"
                className="text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 text-sm font-medium"
                onClick={() => setSetAllStopFlow(null)}
              >
                Close
              </button>
            </div>
            <div className="p-4 overflow-y-auto flex-1 text-sm text-gray-700 dark:text-gray-300 space-y-3">
              {setAllStopFlow.step === "loading" && (
                <p className="flex items-center gap-2">
                  <span className="inline-block w-4 h-4 border-2 border-teal-500 border-t-transparent rounded-full animate-spin" />
                  Loading open positions from Binance…
                </p>
              )}
              {setAllStopFlow.step === "error" && (
                <div className="rounded-lg bg-red-500/10 border border-red-500/30 p-3 text-red-800 dark:text-red-200">
                  {setAllStopFlow.previewError || "Something went wrong."}
                </div>
              )}
              {setAllStopFlow.step === "confirm" && (
                <>
                  <p className="font-medium text-gray-900 dark:text-white">
                    Are you sure you want to update the stop price for all open positions on this side?
                  </p>
                  <p>
                    Pairs to process:{" "}
                    <strong>{setAllStopFlow.count ?? setAllStopFlow.pairs?.length ?? 0}</strong> (single-leg only; hedge
                    symbols are skipped).
                  </p>
                  <p className="text-amber-700 dark:text-amber-300">
                    Estimated time: ~
                    {Math.max(1, (setAllStopFlow.pairs?.length || 0) * SET_ALL_STOP_SEC_PER_PAIR)}s (~
                    {SET_ALL_STOP_SEC_PER_PAIR}s per pair including Binance limits).
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Rules: Heikin Ashi <code className="bg-gray-200/50 dark:bg-gray-700 px-1 rounded">iloc[-2]</code> on the interval selected for each
                    pair below. If no STOP_MARKET on exchange, stop = ha_low (LONG) or ha_high (SHORT). Otherwise set only when HA confirms existing stop.
                  </p>
                  <div className="space-y-2 pt-2">
                    <p className="text-xs font-medium text-gray-600 dark:text-gray-400">Global HA interval (applies to all pairs)</p>
                    <div className="flex flex-wrap gap-2">
                      {HA_STOP_INTERVALS.map((opt) => (
                        <label
                          key={opt}
                          className={`flex items-center gap-2 px-3 py-1 rounded-lg border ${
                            setAllStopFlow.interval === opt
                              ? "bg-teal-600/90 border-teal-700/60 text-white"
                              : "bg-white dark:bg-[#222] border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200"
                          }`}
                        >
                          <input
                            type="radio"
                            name="globalHaStopInterval"
                            value={opt}
                            checked={setAllStopFlow.interval === opt}
                            onChange={() =>
                              setSetAllStopFlow((f) =>
                                f
                                  ? {
                                      ...f,
                                      interval: opt,
                                      pairs: (f.pairs || []).map((pp) => ({ ...pp, interval: opt })),
                                    }
                                  : f
                              )
                            }
                  ...
                  <div className="space-y-2 pt-2">
                    <p className="text-xs font-medium text-gray-600 dark:text-gray-400">Interval per pair</p>
                  </div>
                  {(setAllStopFlow.pairs || []).length > 0 ? (
                    <ul className="overflow-y-auto rounded border border-gray-200 dark:border-gray-600 divide-y divide-gray-200 dark:divide-gray-600 text-xs font-mono">
                      {setAllStopFlow.pairs.map((p, idx) => (
                        <li key={`${p.symbol}-${p.position_side}-${idx}`} className="px-2 py-1">
                          <div className="flex items-center justify-between gap-2">
                            <span className="min-w-0 truncate">
                              {p.symbol} · {p.position_side} · {p.interval || setAllStopFlow.interval || "15m"}
                            </span>
                            <select
                              className="bg-white dark:bg-[#111] border border-gray-200 dark:border-gray-700 text-xs rounded px-2 py-1"
                              value={p.interval || setAllStopFlow.interval || "15m"}
                              onChange={(e) =>
                                setSetAllStopFlow((f) =>
                                  f
                                    ? {
                                        ...f,
                                        pairs: (f.pairs || []).map((pp, i) => (i === idx ? { ...pp, interval: e.target.value } : pp)),
                                      }
                                    : f
                                )
                              }
                    ...
                    <p className="text-gray-500">No positions match this side (or all are hedge pairs).</p>
                  )}
                  <div className="flex gap-2 pt-2">
                    <button
                      type="button"
                      className="flex-1 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white font-medium"
                      onClick={() => setSetAllStopFlow(null)}
                    >
                      Cancel
                    </button>
                    <button
                      type="button"
                      disabled={(setAllStopFlow.pairs || []).length === 0}
                      className="flex-1 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-medium"
                      onClick={() =>
                        setSetAllStopFlow((f) => (f ? { ...f, step: "password" } : f))
                      }
                    >
                      Continue
                    </button>
              ...
              {setAllStopFlow.step === "password" && (
                <form
                  className="space-y-3"
                  onSubmit={async (e) => {
                    e.preventDefault();
                    const pairs = setAllStopFlow.pairs || [];
                    const pw = (setAllStopFlow.password || "").trim();
                    if (!pw) return;
                    setSetAllStopFlow((f) => (f ? { ...f, step: "running", logs: [], currentIndex: 0 } : f));
                    const logs = [];
                    for (let i = 0; i < pairs.length; i++) {
                      ...
                      try {
                        const res = await apiFetch("/api/set-all-stop-one", {
                          method: "POST",
                          ...
                          body: JSON.stringify({
                            ...
                            password: pw,
                            interval: p.interval || setAllStopFlow.interval || "15m",
                          }),
                        });
                        const data = await res.json().catch(() => ({}));
                        const stopVal =
                          data.stop_price != null && data.stop_price !== ""
                            ? Number(data.stop_price).toFixed(6).replace(/\.?0+$/, "")
                            : "";
                        if (data.skipped) {
                          logs.push(
                            `⏭ ${p.symbol}: skipped — ${data.message || "condition not met"}${data.exchange_stop != null ? ` (exchange stop ${data.exchange_stop})` : ""}`
                          );
                        } else if (res.ok && data.ok) {
                          logs.push(`✓ ${p.symbol}: stop set @ ${stopVal} (${data.reason || "ok"})`);
                        } else {
                          logs.push(`✗ ${p.symbol}: ${data.message || res.statusText || "failed"}`);
                        }
                      } catch (err) {
                        logs.push(`✗ ${p.symbol}: ${err?.message || "network error"}`);
                      }
                      setSetAllStopFlow((f) => (f ? { ...f, logs: [...logs], currentIndex: i + 1 } : f));
                      if (i < pairs.length - 1) {
                        await new Promise((r) => setTimeout(r, 2000));
                      }
                    }
                    setSetAllStopFlow((f) => (f ? { ...f, step: "done", logs: [...logs], password: "" } : f));
                  }}
                >
                  ...
                  <input
                    ...
                    className="mt-1 w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-[#111] text-gray-900 dark:text-white"
                    value={setAllStopFlow.password || ""}
                    onChange={(ev) =>
                      setSetAllStopFlow((f) => (f ? { ...f, password: ev.target.value } : f))
                    }
                  />
                  <div className="flex gap-2 pt-2">
                    <button
                      type="button"
                      className="flex-1 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 font-medium"
                      onClick={() => setSetAllStopFlow((f) => (f ? { ...f, step: "confirm" } : f))}
                    >
                      Back
                    </button>
                    <button
                      type="submit"
                      className="flex-1 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white font-medium"
                    >
                      Start ({setAllStopFlow.pairs?.length || 0} pairs)
                    </button>
                  </div>
                </form>
              )}
              {(setAllStopFlow.step === "running" || setAllStopFlow.step === "done") && (
                <>
                  <p className="font-medium text-gray-900 dark:text-white">
                    {setAllStopFlow.step === "running" ? "Processing…" : "Finished"}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {setAllStopFlow.step === "running"
                      ? `Pair ${Math.min((setAllStopFlow.currentIndex || 0) + 1, setAllStopFlow.pairs?.length || 0)} / ${setAllStopFlow.pairs?.length || 0} — ~${Math.max(0, ((setAllStopFlow.pairs?.length || 0) - (setAllStopFlow.currentIndex || 0)) * SET_ALL_STOP_SEC_PER_PAIR)}s remaining (estimate)`
                      : `Processed ${setAllStopFlow.pairs?.length || 0} pair(s).`}
                  </p>
                  <div className="max-h-48 overflow-y-auto rounded border border-gray-200 dark:border-gray-600 p-2 font-mono text-xs whitespace-pre-wrap bg-gray-50 dark:bg-black/40">
                    {(setAllStopFlow.logs || []).join("\n") || (setAllStopFlow.step === "running" ? "…" : "")}
                  </div>
                  {setAllStopFlow.step === "done" && (
                    <button
                      type="button"
                      className="w-full py-2 rounded-lg bg-teal-600 hover:bg-teal-700 text-white font-medium"
                      onClick={() => setSetAllStopFlow(null)}
                    >
                      Close
                    </button>
                  ...
                  <div className="flex items-center gap-10 flex-shrink-0 border-l border-gray-300 dark:border-gray-600 pl-3 ml-1">
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const res = await apiFetch(`/api/set-all-stop-preview?scope=${encodeURIComponent("buy")}`);
                          const data = await res.json().catch(() => ({}));
                          if (!res.ok || data.ok === false) {
                            setSetAllStopFlow((f) =>
                              f ? { ...f, step: "error", previewError: data.message || "Preview failed" } : f
                            );
                            return;
                          }
                          setSetAllStopFlow((f) =>
                            f
                              ? {
                                  ...
                                  pairs: Array.isArray(data.pairs)
                                    ? data.pairs.map((pp) => ({
                                        ...pp,
                                        interval: pp.interval || f.interval || "15m",
                                      }))
                                    : [],
                                  count: typeof data.count === "number" ? data.count : (data.pairs || []).length,
                                  scopeNorm: data.scope || "LONG",
                                }
                              : f
                          );
                        } catch (err) {
                          setSetAllStopFlow((f) =>
                            f ? { ...f, step: "error", previewError: err?.message || "Network error" } : f
                          );
                        }
                      }}
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const res = await apiFetch(`/api/set-all-stop-preview?scope=${encodeURIComponent("sell")}`);
                          const data = await res.json().catch(() => ({}));
                          if (!res.ok || data.ok === false) {
                            setSetAllStopFlow((f) =>
                              f ? { ...f, step: "error", previewError: data.message || "Preview failed" } : f
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (!res.ok) {
                                setSeedError(json.error || "Seed failed");
                                return;
                              }
                              if (json.inserted > 0) await refreshAllData();
                              else setSeedError(json.message || "No rows inserted");
                            } catch (e) {
                          ...
                          className="px-3 py-1.5 rounded bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-amber-950 font-medium text-sm"
                        >
                          {seedLoading ? "Loading…" : "Load demo data"}
                        </button>
                        {seedError && <span className="text-red-700 dark:text-red-300 text-xs">{seedError}</span>}
                      </div>
                    </div>
                  )}
